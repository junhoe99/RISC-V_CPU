...
class b_type_transaction;
    ...
    constraint immediate_constraints {
        ...
        // Full RISC-V B-type immediate range for comprehensive testing
        imm dist {
            [-2048:-1024] := 1,    // Large negative (long backward branch)
            [-1023:-4]    := 2,    // Small negative (backward branch, loops)
            [4:1023]      := 5,    // Small positive (forward branch, conditionals)
            [1024:2047]   := 2     // Large positive (long forward branch)
        };
        
        // Ensure full RISC-V B-type immediate range
        imm >= -2048;  // RISC-V B-type minimum
        imm <= 2047;   // RISC-V B-type maximum (actually 2046 due to alignment)
    }
...
interface assertion_intf(input logic clk, rst);
    ...
    // Property 5: PC underflow detection (warning, not error)
    property pc_underflow_detection;
        @(posedge clk) disable iff (rst)
        (is_branch_instr && branch_taken) |-> (pc_next >= 32'h00000000);
    endproperty
    
    // Property 6: PC overflow detection (very high addresses)
    property pc_overflow_detection;
        @(posedge clk) disable iff (rst)
        (is_branch_instr && branch_taken) |-> (pc_next <= 32'hFFFFFFFC);
    endproperty
    ...
    // PC range monitoring (errors)
    pc_underflow_monitor: assert property(pc_underflow_detection)
        else $error("ASSERTION FAILED: PC underflow - PC went negative (0x%08x) at time %0t", pc_next, $time);
        
    pc_overflow_monitor: assert property(pc_overflow_detection)
        else $error("ASSERTION FAILED: PC overflow - PC went to high address (0x%08x) at time %0t", pc_next, $time);
...
class generator;
    ...
    task body(int count);
        ...
        $display("[GENERATOR] Generated %0d B-type transactions (BEQ, BNE, BLT, BGE, BLTU, BGEU)", test_count);
    endtask
...
class monitor;
    ...
    // Main monitor run task - Transaction based monitoring
    task run();
        ...
        forever begin
            ...
            // Monitor only valid B-type instructions (opcode 7'b1100011)
            if (vif.instr_code[6:0] == 7'b1100011) begin
                ...
                monitored_count++;
                $display("[MONITOR] Monitoring B-type instruction %0d: 0x%08x (funct3=%b) at time %0t - PC=0x%08x", 
                         monitored_count, tr.instruction, tr.funct3, $time, tr.pc_current);
                ...
                // Wait for the instruction to be processed (next clock edge)
                @(posedge vif.clk);
                
                // Exit monitoring loop after 120 B-type instructions processed
                if (monitored_count >= 120) begin
                    $display("[MONITOR] Completed monitoring 120 B-type instructions");
                    break;
                end
            end
        end
...
class scoreboard;
    ...
    int passed_tests = 0;
    int failed_tests = 0;
    int assertion_detected = 0;  // 모든 assertion 통합 카운터
    
    // PC range statistics
    int pc_underflow_count = 0;   // PC < 0 cases
    int pc_overflow_count = 0;    // PC > 0xFFFFFFFC cases
    int pc_normal_count = 0;      // Normal PC range cases
    
    // Instruction generation statistics (by funct3)
    int beq_count = 0;   // BEQ (funct3 = 3'b000)
    int bne_count = 0;   // BNE (funct3 = 3'b001)
    int blt_count = 0;   // BLT (funct3 = 3'b100)
    int bge_count = 0;   // BGE (funct3 = 3'b101)
    int bltu_count = 0;  // BLTU (funct3 = 3'b110)
    int bgeu_count = 0;  // BGEU (funct3 = 3'b111)
    ...
    task run();
        ...
        forever begin
            ...
            // Count instruction types by funct3
            case (tr.funct3)
                3'b000: beq_count++;   // BEQ
                3'b001: bne_count++;   // BNE
                3'b100: blt_count++;   // BLT
                3'b101: bge_count++;   // BGE
                3'b110: bltu_count++;  // BLTU
                3'b111: bgeu_count++;  // BGEU
                default: $display("[SCOREBOARD] Warning: Unknown funct3 value: %b", tr.funct3);
            endcase
            ...
            // PC Range Analysis and Statistics
            if (actual_taken) begin
                if ($signed(actual_pc_next) < 0) begin
                    pc_underflow_count++;
                    assertion_detected++;  // Assertion 감지 카운터 증가
                    $display("🚨 ASSERTION DETECTED: PC UNDERFLOW - PC=0x%08x (negative) with imm=%0d", actual_pc_next, $signed(tr.imm));
                    $display("   This triggers pc_underflow_monitor assertion failure!");
                end else if (actual_pc_next > 32'hFFFFFFFC) begin
                    pc_overflow_count++;
                    assertion_detected++;  // Assertion 감지 카운터 증가
                    $display("🚨 ASSERTION DETECTED: PC OVERFLOW - PC=0x%08x (very high) with imm=%0d", actual_pc_next, $signed(tr.imm));
                    $display("   This triggers pc_overflow_monitor assertion failure!");
                end else begin
                    pc_normal_count++;
                end
            end else begin
                pc_normal_count++;  // Sequential execution is always normal
            end
    ...
    task final_report();
        ...
        $display("Assertion Detected: %0d", assertion_detected);
        $display("");
        $display("=================== INSTRUCTION STATISTICS =================");
        $display("B-type Instruction Generation Count (Target: 120 tests for 6 instructions, ~20 each):");
        $display("BEQ  (funct3=000): %0d tests (%.1f%%)", beq_count, (real'(beq_count) / real'(total_tests)) * 100.0);
        $display("BNE  (funct3=001): %0d tests (%.1f%%)", bne_count, (real'(bne_count) / real'(total_tests)) * 100.0);
        $display("BLT  (funct3=100): %0d tests (%.1f%%)", blt_count, (real'(blt_count) / real'(total_tests)) * 100.0);
        $display("BGE  (funct3=101): %0d tests (%.1f%%)", bge_count, (real'(bge_count) / real'(total_tests)) * 100.0);
        $display("BLTU (funct3=110): %0d tests (%.1f%%)", bltu_count, (real'(bltu_count) / real'(total_tests)) * 100.0);
        $display("BGEU (funct3=111): %0d tests (%.1f%%)", bgeu_count, (real'(bgeu_count) / real'(total_tests)) * 100.0);
        $display("");
        
        // PC Range Statistics
        $display("===================== PC RANGE ANALYSIS =====================");
        $display("PC Normal Range (0x00000000-0xFFFFFFFC): %0d tests (%.1f%%)", 
                 pc_normal_count, (real'(pc_normal_count) / real'(total_tests)) * 100.0);
        $display("PC Underflow (negative):                 %0d tests (%.1f%%)", 
                 pc_underflow_count, (real'(pc_underflow_count) / real'(total_tests)) * 100.0);
        $display("PC Overflow (> 0xFFFFFFFC):              %0d tests (%.1f%%)", 
                 pc_overflow_count, (real'(pc_overflow_count) / real'(total_tests)) * 100.0);
        $display("");
        
        if (pc_underflow_count > 0) begin
            $display("🚨 ASSERTION FAILURES: %0d cases caused PC underflow (negative PC values)", pc_underflow_count);
            $display("   These triggered pc_underflow_monitor assertion failures");
        end
        if (pc_overflow_count > 0) begin
            $display("🚨 ASSERTION FAILURES: %0d cases caused PC overflow (very high PC values)", pc_overflow_count);
            $display("   These triggered pc_overflow_monitor assertion failures");
        end
        $display("============================================================");
        $display("");
        ...
        if (bne_count < min_count) min_count = bne_count;
        if (blt_count < min_count) min_count = blt_count;
        if (bge_count < min_count) min_count = bge_count;
        if (bltu_count < min_count) min_count = bltu_count;
        if (bgeu_count < min_count) min_count = bgeu_count;
        
        if (bne_count > max_count) max_count = bne_count;
        if (blt_count > max_count) max_count = blt_count;
        if (bge_count > max_count) max_count = bge_count;
        if (bltu_count > max_count) max_count = bltu_count;
        if (bgeu_count > max_count) max_count = bgeu_count;
        
        $display("Minimum instruction count: %0d", min_count);
        $display("Maximum instruction count: %0d", max_count);
        $display("Distribution balance: %s", (max_count - min_count <= 5) ? "GOOD" : "UNBALANCED");
        $display("============================================================");
        ...
        if (failed_tests == 0) begin
            if (assertion_detected > 0) begin
                $display("🚨 ALL TESTS PASSED with %0d assertion failures detected! B-type instruction verification successful.", assertion_detected);
                $display("   Assertion failures are PC underflow/overflow cases (see log for details).");
            end else begin
...
class environment;
    ...
    // Main test execution
    task run(int num_tests = 120);
        $display("\n🚀 Starting B-type Instruction Verification Environment");
        $display("Target: %0d test transactions (6 B-type instructions × ~20 tests each)", num_tests);
        ...
        fork
            ...
            begin
                #(num_tests * 200); // Adjusted timeout for 120 tests (24000 time units)
...
module tb_B_type();
    ...
    initial begin
        ...
        `else
            num_tests = 120; // Full test suite - 6 B-type instructions × 20 tests each (minimum target)
        `endif
        ...
        $display("Test Strategy: 120 random tests targeting 6 B-type instructions");
        $display("             (BEQ, BNE, BLT, BGE, BLTU, BGEU - aim for ~20 tests each)");
        $display("==================================================");
        ...
        fork
            env.run(num_tests);
            begin
                #10000000; // 10ms timeout for 120 tests
